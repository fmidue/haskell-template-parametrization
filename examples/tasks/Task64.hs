...
test =
  [ " correct program behavior?" ~:
    qcWithTimeoutAndArgs 5000000 stdArgs{maxSuccess=250, maxSize=50} $
...
specification =
  ...
  readInput "x" ints <>
  tillExit (
    branch ((\xs -> Last xs :+ Last (Init xs) :== Lit 0 ) $ getAll @Int "x")
    (optionalTextOutput <>
     readInput "x" ints)
    exit
  ) <>
  writeOutput [anything <> var 0 <> anything] [Length . listFilter $ getAll @Int "x"]

listFilter :: Term [Int] -> Term [Int]
listFilter = Filter predicate
