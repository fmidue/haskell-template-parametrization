...
test = [
       [ " example" ~:
            qcWithTimeout 5000 Solution.example
       , " insert (random inputs)" ~:
            qcWithTimeout 5000 $ \(x :: Int) (L xs) -> Solution.insert x xs == insert x xs
       , " insertionSort (random inputs)" ~:
            qcWithTimeout 5000 $ \(xs :: [Int]) -> Solution.insertionSort xs == sort xs
